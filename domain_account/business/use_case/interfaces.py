...
class UseCase(Generic[T_input, T_output_co, T_service], metaclass=ABCMeta):
    """
    ...
    Args:
        service (T_service): The service object(s) implemented in Frameworks layer required by the use case to execute its logic.

    Attributes:
        __service (T_service): The service object(s) required by the use case to execute its logic.
    ...
    def __init__(self, service: T_service) -> None:
        ...
        Args:
            service (T_service): The service object(s) required by the use case to execute its logic.
